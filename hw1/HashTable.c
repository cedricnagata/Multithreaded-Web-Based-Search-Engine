...
static bool BucketFindKey(HTKey_t key, HTKeyValue_t **payload, LLIterator *itr);
...
bool HashTable_Insert(HashTable *table,
  ...
  // Allocate space for new keyvalue to add
  ...
  // assign values for payload
  new_payload->key = newkeyvalue.key;
  new_payload->value = newkeyvalue.value;

  // no elements case
  if (LinkedList_NumElements(chain) == 0) {
    LinkedList_Push(chain, (void*) new_payload); 
    table->num_elements++;

    return false;
  }

  LLIterator *itr = LLIterator_Allocate(chain);
  ...
  // return parameter
  HTKeyValue_t *rtn_payload;

  // if key exists
  if (BucketFindKey(newkeyvalue.key, &rtn_payload, itr)) {
    // set return parameter
    oldkeyvalue->key = rtn_payload->key;
    oldkeyvalue->value = rtn_payload->value;

    // change to new value
    rtn_payload->value = newkeyvalue.value;
    ...
    LLIterator_Free(itr);
    LinkedList_Append(chain, (void *) new_payload);
...
bool HashTable_Find(HashTable *table,
  ...
  // if key exists
  if (BucketFindKey(key, &rtn_payload, itr)) {
    
    // set return parameter
    keyvalue->key = rtn_payload->key;
    keyvalue->value = rtn_payload->value;
...
bool HashTable_Remove(HashTable *table,
  ...
  if (BucketFindKey(key, &rtn_payload, itr)) {
    ...
    // remove keyvalue
    LLIterator_Remove(itr, &LLNoOpFree);
    LLIterator_Free(itr);
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  // ht must have elements
  if (iter->ht->num_elements == 0) {
    return false;
  }

  // if ll iterator is valid ht iterator is valid
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // iterator is past end or not valid
  if (!HTIterator_IsValid(iter)) {
    return false;
  }

  // if ll iterator next is valid
  ...
  // if no new buckets are available
  return false;
}
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  // return parameter
  HTKeyValue_t *rtnPayload;

  if (HTIterator_IsValid(iter)) {
    // get ll iterator keyvalue
    LLIterator_Get(iter->bucket_it, (void**) &rtnPayload);

    // set return parameter
    keyvalue->key = rtnPayload->key;
    keyvalue->value = rtnPayload->value;

    return true;
  }

  // if ht iterator is not valid
  return false;
}
...
// Helper function to find a KeyValue within a Hashtable bucket

// Arguments:
//  - key to find
//  - return parameter of KeyValue found
//  - LinkedList Iterator to find key within
// 
// Return:
//  - true if key found
//  - false if key not found
static bool BucketFindKey(HTKey_t key, HTKeyValue_t **rtnKeyValue, LLIterator *itr) {
  // iterate through ll
  while (LLIterator_IsValid(itr)) {
    LLIterator_Get(itr, (void**) rtnKeyValue);

    // if current key equals key to find
    if ((*rtnKeyValue)->key == key) {
      return true;
    }
