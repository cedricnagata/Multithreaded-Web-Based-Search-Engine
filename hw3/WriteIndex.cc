...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  for (int i = 0; i < doctable_bytes + memidx_bytes; i++) {
    if (fread(&next_char, sizeof(next_char), 1, f) != 1) {
      return kFailedWrite;
    }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    IndexFileOffset_t chain_len = LinkedList_NumElements(ll);

    int bytes_written = WriteHTBucketRecord(f, record_pos, chain_len, bucket_pos);

    if (bytes_written == 0) {
      return kFailedWrite;
    }

    record_pos += bytes_written;

    if (chain_len != 0) {
      bytes_written = WriteHTBucket(f, bucket_pos, ll, fn);
      if (bytes_written == 0) {
        return kFailedWrite;
      }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    ElementPositionRecord element_pos_rec(element_pos);

    element_pos_rec.ToDiskFormat();
    if (fseek(f, record_pos, SEEK_SET) != 0) {
      return kFailedWrite;
    }

    if (fwrite(&element_pos_rec, sizeof(ElementPositionRecord), 1, f) != 1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
    ...
    IndexFileOffset_t element_bytes = fn(f, element_pos, kv);
    if (element_bytes == -1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  DocIDElementHeader doc_id_header(doc_id, num_positions);
  doc_id_header.ToDiskFormat();
  if (fwrite(&doc_id_header, sizeof(DocIDElementHeader), 1, f) != 1) {
    return kFailedWrite;
  }
