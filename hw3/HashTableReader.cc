...
HashTableReader::LookupElementPositions(HTKey_t hash_key) const {
  ...
  for (int i = 0; i < bucket_rec.chain_num_elements; i++) {
    ElementPositionRecord el_pos;
    Verify333(fseek(file_, bucket_rec.position + (sizeof(ElementPositionRecord) * i), SEEK_SET) == 0);
    Verify333(fread(&el_pos, sizeof(ElementPositionRecord), 1, file_) == 1);
    
    el_pos.ToHostFormat();
    ret_val.push_back(el_pos.position);
  }
