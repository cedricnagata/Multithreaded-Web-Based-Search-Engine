...
static void GetPortAndPath(int argc,
  ...
  *path = argv[2];
  struct stat buf;
  if (stat((*path).c_str(), &buf) == -1 || !S_ISDIR(buf.st_mode)) {
    cerr << "File path is invalid" << endl;
    Usage(argv[1]);
  }
  ...
  for (int i = 3; i < argc; i++) {
    ...
    // push indices with file name
    indices->push_back(file_name);
  }

  // if no indices found
  if (indices->size() == 0) {
    cerr << "No reasonable index files found" << endl;
    Usage(argv[1]);
  }
}
