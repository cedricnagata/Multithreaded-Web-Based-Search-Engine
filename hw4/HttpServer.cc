...
static void HttpServer_ThrFn(ThreadPool::Task* t) {
  ...
  // STEP 1:
  bool done = false;
  HttpConnection hc(hst->client_fd); // get connection

  while (!done) {
    
    // get next request if it doesn't exist close client
    HttpRequest req;
    ...
    // if connection val is close then close client
    ...
    // if response cannot be written close client
...
static HttpResponse ProcessFileRequest(const string& uri,
  ...
  // STEP 2:
  URLParser url_par;
  url_par.Parse(uri); // parse uri

  file_name = url_par.path().substr(8); // get path from url parsed

  FileReader reader(base_dir, file_name);
  string contents;

  if (reader.ReadFile(&contents)) { // get contents
    ret.AppendToBody(contents); // append contents to response body

    // get suffix
    size_t suffix_pos = file_name.rfind(".");
    string suffix = file_name.substr(suffix_pos);

    // content headers for suffix
    ...
    // set code, protocal and message for OK
  ...
  ret.set_message("File Not Found");
...
static HttpResponse ProcessQueryRequest(const string& uri,
  ...
  if (uri.find("query?terms=") != string::npos) {
    ...
    string query = url_par.args()["terms"]; // get query 
    boost::trim(query);
    boost::to_lower(query);

    // get words from query
    std::vector<string> query_split;
    boost::split(query_split, query, boost::is_any_of(" "), boost::token_compress_on);

    // process query
    hw3::QueryProcessor query_process(indices, false);
    std::vector<hw3::QueryProcessor::QueryResult> query_results = query_process.ProcessQuery(query_split);

    if (query_results.size() == 0) { // if no results found
      ret.AppendToBody("<p>\r\n");
      ret.AppendToBody("No results found for \"" + EscapeHtml(query) + "\"<p>\r\n\r\n");

    } else { // list out results
      
      // num of results
      ret.AppendToBody("<p>\r\n");
      ret.AppendToBody(std::to_string(query_results.size()) + " results" + 
                       " found for \"" + EscapeHtml(query) + "\"<p>\r\n\r\n");
      ret.AppendToBody("<ul>\r\n");

      // list links to results and rank
      for (size_t i = 0; i < query_results.size(); i++) {
        ret.AppendToBody(" <li> <a href=\"");

        // if webpage
        if (query_results[i].document_name.substr(0, 7) != "http://") {
          ret.AppendToBody("/static/");
        }

        ret.AppendToBody(query_results[i].document_name + "\">" + 
                         EscapeHtml(query_results[i].document_name) + 
                         "</a> [" + std::to_string(query_results[i].rank) + "]");
      }
