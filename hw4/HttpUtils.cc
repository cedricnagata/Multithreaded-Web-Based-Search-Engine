...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // check root_dir is directory path
  if (!realpath(root_dir.c_str(), root_dir_buf)) {
    return false;
  }

  // check test_file is path to file
  if (!realpath(test_file.c_str(), test_file_buf)) {
    return false;
  }

  // check if test_file is within rootdir
  if (strlen(test_file_buf) <= strlen(root_dir_buf)) {
    return false;
  }

  // split root and file paths into parts, compare last part of root path
  // with corresponding part of file path
  std::vector<string> root;
  std::vector<string> file;

  boost::algorithm::split(root, root_dir_buf, boost::is_any_of("/"), boost::algorithm::token_compress_on);
  boost::algorithm::split(file, test_file_buf, boost::is_any_of("/"), boost::algorithm::token_compress_on);

  std::string root_end = root[root.size() - 1];
  std::string file_end = file[root.size() - 1];

  if (root_end.compare(file_end) != 0) {
    return false;
  }
