...
#include <memory> // FIX: added memory library

int main(int argc, char **argv) {
  // Allocate an integer on the heap, initialize to value 5.
  std::unique_ptr<int> x (new int(5)); // FIX: add unique pointer to initialize int
  // OLD: int *x = new int(5);
  std::cout << "*x is: " << *x << std::endl;

  // Allocate a vector of integers on the heap, add some values to
  // that vector, sort the vector, print the values.
  std::unique_ptr<std::vector<int>> v (new std::vector<int>); // FIX: declare unique pointer vector
  // OLD: std::vector<int> *v = new std::vector<int>; 
  v->push_back(13);
  v->push_back(42);
  v->push_back(17);
  ...
  std::vector<std::unique_ptr<int>> v2; // FIX: declare vector of unique pointer ints
  /****FIX: push unique pointer ints to vector****/
  ...
  // OLD: for (int *el : v2) { 
  for (std::unique_ptr<int> &el : v2) { // FIX: unique pointer for vector elements in loop
    std::cout << *el << " ";
  }
  std::cout << std::endl;
