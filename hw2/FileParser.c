...
static void InsertContent(HashTable* tab, char* content) {
  ...
  while (true) {
    if (*cur_ptr == '\0') {
      break;
    } else if (isalpha(*cur_ptr) != 0) { // test if alphabetic
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    // insert old key in table
    HTKeyValue_t old_key_value;
    HashTable_Insert(tab, kv, &old_key_value);
  }
}
