...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  postings_kv.value = (void *) postings;
...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  while (HTIterator_IsValid(itr)) {
    Verify333(HTIterator_Get(itr, &kv));

    // allocate search result
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) { // loop through docs in postings list
      SearchResult *res;
      LLIterator_Get(ll_it, (void **) &res);

      // if found add to rank if not remove doc
      if (HashTable_Find(wp->postings, res->doc_id, &kv)) {
        res->rank += LinkedList_NumElements((LinkedList*) kv.value); // add rank
